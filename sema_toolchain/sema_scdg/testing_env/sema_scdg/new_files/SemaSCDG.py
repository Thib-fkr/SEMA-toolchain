...
class SemaSCDG():
    ...
    def setup_heap(self, state, proj):
        ...
        SHARED_USER_DATA_ADDR = 0x7ffe0000
        def watch_SHARED_USER_DATA(bp_state):
            addr_sym = bp_state.inspect.mem_read_address
            if hasattr(addr_sym, 'symbolic') and addr_sym.symbolic:
                return

            addr = bp_state.solver.eval(addr_sym)
            if addr != 0 and addr in list(map(lambda a: SHARED_USER_DATA_ADDR + a, [0x2d4])):
                print("[+] \t\t found mem bp KD_SHARED_USER_DATA")
                self.mem_bp_evasion.append(("KD_SHARED_USER_DATA", hex(addr), hex(addr-SHARED_USER_DATA_ADDR)))

        state.inspect.b("mem_read", when=angr.BP_BEFORE, action=watch_SHARED_USER_DATA)
        ...
        if proj.arch.name == "AMD64":
            ...
            def watch_PEB(bp_state):
                addr_sym = bp_state.inspect.mem_read_address
                if hasattr(addr_sym, 'symbolic') and addr_sym.symbolic:
                    return

                addr = bp_state.solver.eval(addr_sym)
                peb_addr_conc = bp_state.solver.eval(peb_addr)
                if addr != 0 and addr in list(map(lambda a: peb_addr_conc + a, [0x2, 0xbc, 0x30])):
                    print("[+] \t\t found mem bp PEB")
                    self.mem_bp_evasion.append(("PEB", hex(addr), hex(addr-peb_addr_conc)))

            state.inspect.b("mem_read", when=angr.BP_BEFORE, action=watch_PEB)
        else:
